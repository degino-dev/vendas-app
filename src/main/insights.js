  ...
  Object.keys(porCliente).forEach(function (cid) {
    ...
    if (intervalo && diasDesde > Math.round(intervalo * 1.5) && diasDesde > 15) {
      ...
      dicas.push({
        ...
        texto: 'Nao compra ha ' + diasDesde + ' dias (padrao dele: a cada ' + intervalo + ' dias). Vale contato urgente para reativar.',
        detalhe: 'Ultima compra: ' + formatarData(ultima) + ' | Ticket medio: ' + formatarMoeda(ticket) + (whats ? ' | ' + whats : '')
      })
      return
    }
    ...
    if (intervalo && diasDesde >= intervalo) {
      dicas.push({
        ...
        texto: 'Costuma comprar a cada ' + intervalo + ' dias. Ultima compra foi ha ' + diasDesde + ' dias. Esta semana e o momento ideal para contato.',
        detalhe: 'Ultima compra: ' + formatarData(ultima) + ' | Ticket medio: ' + formatarMoeda(ticket) + (whats ? ' | ' + whats : '')
      })
      return
    }
    ...
    if (semanaComum && datas.length >= 2 && semanaComum === semanaAtual) {
      dicas.push({
        ...
        tipo: 'padrao',
        clienteId: cid,
        titulo: 'Padrao de compra: ' + nome,
        texto: 'Costuma comprar na semana ' + semanaComum + ' do mes. Estamos nela - boa hora para ligar.',
        detalhe: 'Ticket medio: ' + formatarMoeda(ticket) + (whats ? ' | ' + whats : '')
      })
      return
    }
    ...
    if (temInsumo && !temEquipamento && lista.length >= 2) {
      dicas.push({
        ...
        texto: 'So compra insumos (' + lista.length + ' compras). Talvez seja hora de oferecer equipamento.',
        detalhe: 'Ticket medio: ' + formatarMoeda(ticket) + (whats ? ' | ' + whats : '')
      })
    }
  })
...
export function atualizarEstado(estado, acao, chave) {
  ...
  if (acao === 'ver') {
    ...
    novo.adiados[chave] = amanha.toISOString().slice(0, 10)
  }
  return novo
}
...
export function gerarEstatisticasCliente(opcoes) {
  ...
  var nomeDia = ['domingo', 'segunda-feira', 'terca-feira', 'quarta-feira', 'quinta-feira', 'sexta-feira', 'sabado']
